...
export default function OTPInput({ value, valueLength, onChange }: Props) {
  ...
  return (
    <div className="flex gap-2 outline-none rounded-lg max-w-[350px]">
      {valueItems.map((digit, idx) => (
        <input
          ...
          className="text-[var(--foreground)] p-1 w-full h-8/10 border rounded-sm transition duration-400 text-center text-2xl outline-0 font-bold leading-1 focus-within:ring-2"
          pattern="\d{1}"
        />
      ))}
    </div>
